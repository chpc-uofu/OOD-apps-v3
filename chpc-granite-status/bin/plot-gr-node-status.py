...
TITLE_DICT = dict(
  text=f'<b>Granite Node Status</b> {now} <br>{stat_str}',
  y=0.900,
  x=0.0,
  xanchor='left',
  yanchor='top',
...
if __ANIM_MODE__:
  TITLE_DICT['text'] = f'<b>Granite Node Status</b> {datafile.split("/")[-1].split(".")[0].split("node-status-")[-1]} <br>{stat_str}'
...
fig.layout.height = BASE_DEPTH * (M+0.6)  # /11) if N > 11 else BASE_DEPTH
print('len M N w h')
