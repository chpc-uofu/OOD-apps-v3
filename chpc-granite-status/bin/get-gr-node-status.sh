...
readonly working_dir="/uufs/chpc.utah.edu/sys/ondemand/chpc-apps/chpc-granite-status"
cd "$working_dir"

readonly density_limit=10 # save one per ten minutes
readonly snapshot_dir="/uufs/chpc.utah.edu/sys/ondemand/node-usage-snapshots/snapshot-gr"
...
paste                                   \
  <(/uufs/granite/sys/installdir/slurm/std/bin/sinfo -M granite --Node -o "$sinfo_short_fmt" | grep -v CLUSTER | sed -e s/\"/\\\\\"/g -e s/\'/\"/g ) \
  <(/uufs/granite/sys/installdir/slurm/std/bin/sinfo -M granite --Node -O "$sinfo_long_fmt" | grep -v CLUSTER  ) \
  > "$snapshot_dat"
./bin/plot-gr-node-status.py "$snapshot_dat" "$snapshot_job_stats_dat"
