...
function toggle_advanced_options() {
  ...
  if(advanced_options.checked === true)  {
      nodes_checkbox.parent().show();
      if (num_nodes[0].value === "1") {
      ...
      if ((num_nodes[0].value === "1") && ( memtask[0].value === "0" || memtask[0].value === "" ) && ( gpu_type[0].value === "none" ) && ( nodelist[0].value === "" ) && ( additional_environment[0].value === "" ) && ( constraint[0].value === "")) {
      nodes_checkbox.parent().hide();
      num_nodes.parent().hide();
      memtask_checkbox.parent().hide();
      memtask.parent().hide();
      gpu_checkbox.parent().hide();
      gpu_type.parent().hide();
      gpu_count.parent().hide();
      ...
      nodes_checkbox[0].checked = false;
      memtask_checkbox[0].checked = false;
      gpu_checkbox[0].checked = false;
...
function toggle_nodes() {
  let num_nodes = $('#batch_connect_session_context_num_nodes');
  let nodes_checkbox = document.getElementById("batch_connect_session_context_nodes_checkbox");

  if(nodes_checkbox.checked === true)  {
      num_nodes.parent().show();
  } else {
      if (num_nodes[0].value === "1") {
        num_nodes.parent().hide();
      } else {
        nodes_checkbox.checked = true;
        num_nodes.parent().show();
      }
  }
}
...
$(document).ready(function () {
	...
	queue.change(function () {
        ...
        // toggle num_nodes on form refresh
        let install_trigger7 = $('#batch_connect_session_context_nodes_checkbox');
        install_trigger7.change(toggle_nodes);
        const form7 = document.getElementById("new_batch_connect_session_context");
        toggle_nodes();
        ...
        toggle_memtask();
        // toggle gpu on form refresh
        let install_trigger6 = $('#batch_connect_session_context_gpu_checkbox');
        install_trigger6.change(toggle_gpu);
        const form6 = document.getElementById("new_batch_connect_session_context");
        toggle_gpu();
