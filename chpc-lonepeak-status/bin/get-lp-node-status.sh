...
readonly working_dir="/uufs/chpc.utah.edu/sys/ondemand/chpc-apps/chpc-lonepeak-status"
cd "$working_dir"

readonly density_limit=10 # save one per ten minutes
readonly snapshot_dir="/uufs/chpc.utah.edu/sys/ondemand/node-usage-snapshots/snapshot-lp"
...
paste                                   \
  <(/uufs/notchpeak.peaks/sys/installdir/slurm/std/bin/sinfo -M lonepeak --Node -o "$sinfo_short_fmt" | grep -v CLUSTER | sed -e s/\"/\\\\\"/g -e s/\'/\"/g ) \
  <(/uufs/notchpeak.peaks/sys/installdir/slurm/std/bin/sinfo -M lonepeak --Node -O "$sinfo_long_fmt" | grep -v CLUSTER ) \
  > "$snapshot_dat"
./bin/plot-lp-node-status.py "$snapshot_dat" "$snapshot_job_stats_dat"
