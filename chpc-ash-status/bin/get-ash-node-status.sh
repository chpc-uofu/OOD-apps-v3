...
readonly snapshot_dir="/uufs/chpc.utah.edu/sys/ondemand/node-usage-snapshots/snapshot-ash"
...
paste                                   \
  <(/uufs/notchpeak.peaks/sys/installdir/slurm/std/bin/sinfo -M ash --Node -o "$sinfo_short_fmt" | grep -v CLUSTER | sed -e s/\"/\\\\\"/g -e s/\'/\"/g ) \
  <(/uufs/notchpeak.peaks/sys/installdir/slurm/std/bin/sinfo -M ash --Node -O "$sinfo_long_fmt" | grep -v CLUSTER ) \
  > "$snapshot_dat"
./bin/plot-ash-node-status.py "$snapshot_dat" "$snapshot_job_stats_dat"
